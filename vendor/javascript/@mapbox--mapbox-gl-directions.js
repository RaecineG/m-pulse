 ...
 * @param {Object} map The mapboxgl instance
 * @private
 */class Instructions{constructor(t,e,i,n){this.container=t;this.actions=i;this.store=e;this._map=n;this.directions={};this.render()}render(){this.store.subscribe((()=>{const{hoverMarker:t,setRouteIndex:e}=this.actions;const{routeIndex:i,unit:n,directions:o,error:r,compile:s}=this.store.getState();const a=!l(o[i],this.directions);if(r)this.container.innerHTML=j({error:r});else if(o.length&&a){const r=this.directions=o[i];const a=p.getAllSteps(r);s&&r.legs.forEach((function(t){t.steps.forEach((function(t){t.maneuver.instruction=s("en",t)}))}));this.container.innerHTML=L({routeIndex:i,routes:o.length,steps:a,format:p.format[n],duration:p.format.duration(r.duration),distance:p.format[n](r.distance)});const c=this.container.querySelectorAll(".mapbox-directions-step");Array.prototype.forEach.call(c,(e=>{const i=e.getAttribute("data-lng");const n=e.getAttribute("data-lat");e.addEventListener("mouseover",(()=>{t([i,n])}));e.addEventListener("mouseout",(()=>{t(null)}));e.addEventListener("click",(()=>{this._map.flyTo({center:[i,n],zoom:16})}))}));const u=this.container.querySelectorAll('input[type="radio"]');Array.prototype.forEach.call(u,(t=>{t.addEventListener("change",(t=>{e(parseInt(t.target.id,10))}))}))}else this.container.innerHTML&&a&&(this.container.innerHTML="")}))}}
/**
